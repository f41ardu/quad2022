...
byte MPU6050::begin(int gyro_config_num, int acc_config_num){
  ...
  preInterval = micros(); // may cause lack of angular accuracy if begin() is much before the first update()
  return status;
}
...
*/
byte MPU6050::setFilterBandwidth(int config_num){
  byte status;
  switch(config_num){ // BW 256 Hz (not recommended)
    case 0: // 
    status = writeData(MPU6050_CONFIG_REGISTER, 0x00);
    break;
  case 1: // BW 188 Hz
    status = writeData(MPU6050_CONFIG_REGISTER, 0x01);
    break;
  case 2: // BW 98 Hz
    status = writeData(MPU6050_CONFIG_REGISTER, 0x2);
    break;
  case 3: // BW 42 Hz
    status = writeData(MPU6050_CONFIG_REGISTER, 0x3);
    break;
  case 4: // BW 20 Hz
    status = writeData(MPU6050_CONFIG_REGISTER, 0x4);
    break;
  case 5: // BW  10 Hz
    status = writeData(MPU6050_CONFIG_REGISTER, 0x5);
    break;
  case 6: // BW 5 Hz
    status = writeData(MPU6050_CONFIG_REGISTER, 0x6);
    break;
  default: // error
    status = 1;
    break;
...
void MPU6050::update(){
  ...
  unsigned long Tnew = micros(); // millis();
  ...
  angleY = wrap(filterGyroCoef*(angleAccY + wrap(angleY + sgZ*gyroY*dt - angleAccY, 180)) + (1.0-filterGyroCoef)*angleAccY, 180);
  angleZ += gyroZ*dt; // not wrapped
  
  // To dampen the pitch and roll angles a complementary filter is used
  measuresR  = measuresR  * 0.98 + gyroX * 0.2;
  measuresP = measuresP * 0.98 + gyroY * 0.2;
  measuresY   = -gyroZ;        // Store the angular motion for this axis
 
  
  // Apply low-pass filter (10Hz cutoff frequency)
  amotionR  = 0.7 * amotionR  + 0.3 * gyroX;        // Roll
  amotionP = 0.7 * amotionP + 0.3 * gyroY;          // Pitch
  amotionY   = 0.7 * amotionY   + 0.3 * gyroZ;      // Yaw
